...
Graph = namedtuple('Graph', ['g', 'src', 'tgt', 'tgt_y', 'nids', 'eids',
                             'nid_arr', 'n_nodes', 'n_edges', 'n_tokens'])
...
class GraphPool:
    ...
    def beam(self, src_buf, start_sym, max_len, k, device='cpu'):
        ...
        for src_sample, n, n_ee, n_ed, n_dd in zip(src_buf, src_lens, num_edges['ee'], num_edges['ed'],
                                                   num_edges['dd']):
            for _ in range(k):
    ...
    def __call__(self, src_buf, tgt_buf, device='cpu'):
        ...
        n_nodes, n_edges, n_tokens = 0, 0, 0
        for src_sample, tgt_sample, n, m, n_ee, n_ed, n_dd in zip(src_buf, tgt_buf, src_lens, tgt_lens, num_edges['ee'],
                                                                  num_edges['ed'], num_edges['dd']):
            src.append(th.tensor(src_sample, dtype=th.long, device=device))
